...
bool imageCreate(VkImage& image, VkDevice device, VkFormat format,
    ...
    if (VK_SUCCESS != vkCreateImage(device, &info, nullptr, &image)) {
        return 0;
    }
...
bool imageViewCreate(VkImageView& imageView, VkDevice device, VkFormat format,
    ...
    if (VK_SUCCESS != vkCreateImageView(device, &info, nullptr, &imageView)) {
        return 0;
    }
...
bool allocateImage(AllocatedImage& allocatedImage, VkDevice device,
    ...
    VkImageCreateInfo rimg_info = imageCreateInfo(
        allocatedImage.imageFormat, usage, allocatedImageExtent);
    ...
    vmaCreateImage(allocator, &rimg_info, &rimg_allocinfo, &allocatedImage.image,
