...
namespace vkr {
void PBRPass::buildPipeline() {
    auto device = app.system.device;
    const auto& depthImage = app.depthImage;
    const auto& drawImage = app.drawImage;

    VkShaderModule triangleFragShader =
        vk::loadShaderModule(RESPATH "/shaders/pbr.frag.spv", device);

    VkShaderModule triangleVertexShader =
        vk::loadShaderModule(RESPATH "/shaders/pbr.vert.spv", device);
...
void PBRPass::buildPipelineLayout() {
...
PBRPass::PBRPass() : sceneState(SceneState::get()) {
    buildPipelineLayout();
    buildPipeline();
}
PBRPass::~PBRPass() {
...
void PBRPass::render(VkCommandBuffer cmd) {
